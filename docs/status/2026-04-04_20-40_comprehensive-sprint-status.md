...
2. **`shouldSkipDir` is disconnected from `WithIgnoreDirs`** — This is a design flaw, not a bug. `WithIgnoreDirs` adds a *filter* (post-walk) but `shouldSkipDir` (pre-walk) only checks `DefaultIgnoreDirs`. The fix needs a new field `userSkipDirs []string` populated during option application, merged in `shouldSkipDir`.
