...
func TestFilterGeneratedCodeFull_WithContent(t *testing.T) {
	...
	// Create a file that looks like sqlc by filename but isn't
	regularFile := tmpDir + "/models.go"
	if err := writeFile(regularFile, []byte("package main\n\nfunc main() {}")); err != nil {
		t.Fatalf("Failed to create test file: %v", err)
	}

	// Create a file that IS sqlc generated (with content marker)
	...
	// File without sqlc content marker should pass when content check is enabled
	regularEvent := Event{Path: regularFile, IsDir: false}
	if !filter(regularEvent) {
		t.Errorf("Expected non-sqlc file to pass with content check, got filtered")
	}

	// File with sqlc content marker should be filtered
	sqlcEvent := Event{Path: sqlcFile, IsDir: false}
	if filter(sqlcEvent) {
		t.Errorf("Expected sqlc file to be filtered with content check")
	}
}
...
func writeFile(path string, content []byte) error {
	return writeFileWithMode(path, content, 0o644)
}
