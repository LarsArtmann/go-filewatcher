...
- **`MIGRATION.md` not updated.** The `API_STABILITY.md` breaking-change policy explicitly says: *"Migration guide: Document how to migrate in `MIGRATION.md`."* I put migration snippets in the `API_STABILITY.md` table instead. `MIGRATION.md` exists and currently only covers v1→v2. The v2.3→v3 deprecation migrations belong there.
...
5. **Scope creep avoided:** I correctly resisted deprecating the thin convenience wrappers (`WithExtensions`, `WithIgnoreDirs`, etc.) and documented *why* they're kept. Good restraint.
