...
func TestNewWatcherError(t *testing.T) {
	...
	err := NewWatcherError(NewOpString("test_op"), "/test/path", ErrPathNotFound)

	if err.Op != NewOpString("test_op") {
		t.Errorf("Op = %q, want %q", err.Op, NewOpString("test_op"))
	}
